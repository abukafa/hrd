<?php

namespace App\Controllers;

use App\Models\SkimModel;
use App\Models\SantriModel;
use App\Models\PotonganModel;
use App\Models\TunjanganModel;
use App\Controllers\BaseController;
use App\Models\KompetensiModel;
use App\Models\UserModel;
use PhpOffice\PhpSpreadsheet\Style\Fill;
use PhpOffice\PhpSpreadsheet\Style\Border;
use PhpOffice\PhpSpreadsheet\Spreadsheet;
use PhpOffice\PhpSpreadsheet\Writer\Xlsx;

class OfficePengurus extends BaseController
{
    protected $userModel;
    protected $santriModel;
    protected $tunjanganModel;
    protected $kompetensiModel;

    public function __construct()
    {
        $this->userModel = new UserModel();
        $this->santriModel = new SantriModel();
        $this->kompetensiModel = new KompetensiModel();
        $this->skimModel = new SkimModel();
        $this->tunjanganModel = new TunjanganModel();
        $this->potonganModel = new PotonganModel();
    }

    public function index()
    {
        // $keyword = $this->request->getVar('keyword');
        // if ($keyword) {
        //     $santri = $this->santriModel->search($keyword);
        // } else {
        //     $santri = $this->santriModel;
        // }

        $data = [
            'santri' => $this->santriModel->where('status_santri <> ', 'Suspend')->findAll(),
            'user' => $this->userModel->where('uname', session()->get('uname'))->first()
        ];
        return view('office/pengurus', $data);
    }

    public function detail($id)
    {
        if ($id) {
            $santri = $this->santriModel->asArray()->find($id);
            $kompetensi = $this->kompetensiModel->where('nip', $santri['nip'])->findAll();
            $tunjangan = $this->tunjanganModel->where('nip', $santri['nip'])->first();
            $data = [
                'santri' => $santri,
                'kompetensi' => $kompetensi,
                'tunj' => $tunjangan
            ];
            // dd($data);
            return view('office/pengurus_detail', $data);
        } else {
            return view('office/pengurus_detail');
        }
    }

    public function ceklis($id)
    {
        if ($this->request->isAJAX()) {
            $santri =  $this->santriModel->find($id);

            if ($santri['acc']) {
                $this->santriModel->set('acc', false);
                $acc = false;
            } else {
                $this->santriModel->set('acc', true);
                $acc = true;
            }

            $this->santriModel->where('id', $id);
            $this->santriModel->update();

            echo json_encode($acc);
        } else {
            exit('Maaf Tidak dapat diproses');
        }
    }

    public function add_tunj($id, $tunj)
    {
        if ($this->request->isAJAX()) {
            $santri =  $this->santriModel->find($id);

            $santri = ['grade' => $santri['grade'], 'tunjang' => $santri[$tunj]];
            $skim = $this->skimModel->find($santri['grade']);

            switch ($tunj) {
                case 't_jab':
                    $data = ['tunjangan' => $skim['t_jab']];
                    break;
                case 't_stt':
                    $data = ['tunjangan' => $skim['t_stt']];
                    break;
                case 't_ank':
                    $data = ['tunjangan' => $skim['t_ank']];
                    break;
                case 't_prg':
                    $data = ['tunjangan' => $skim['t_prg']];
                    break;
                case 't_kes':
                    $data = ['tunjangan' => $skim['t_kes']];
                    break;
                default:
                    $data = ['tunjangan' => 0];
                    break;
            }


            if ($santri['tunjang']) {
                $mydata = ['tunjangan' => 0];
                $this->santriModel->save(['id' => $id, $tunj => false]);
                $this->tunjanganModel->save(['id' => $id, $tunj => $mydata['tunjangan']]);
            } else {
                $mydata = $data;
                $this->santriModel->save(['id' => $id, $tunj => true]);
                $this->tunjanganModel->save(['id' => $id, $tunj => $mydata['tunjangan']]);
            }

            echo number_format($mydata['tunjangan'], 0, ".", ",");
            // echo json_encode($mydata);
        } else {
            exit('Maaf Tidak dapat diproses');
        }
    }

    public function insert()
    {
        $this->santriModel->save([
            'nip' => $this->request->getVar('nip'),
            'no_absen' => $this->request->getVar('no_absen'),
            'ktp' => $this->request->getVar('ktp'),
            'gender' => $this->request->getVar('gender'),
            'nama' => $this->request->getVar('nama'),
            'panggilan' => $this->request->getVar('panggilan'),
            'tmp_lahir' => $this->request->getVar('tmp_lahir'),
            'tgl_lahir' => $this->request->getVar('tgl_lahir'),
            'gol_darah' => $this->request->getVar('gol_darah'),
            'status' => $this->request->getVar('status'),
            'pasangan' => $this->request->getVar('pasangan'),
            'jml_istri' => $this->request->getVar('jml_istri'),
            'jml_anak' => $this->request->getVar('jml_anak'),
            'pendidikan' => $this->request->getVar('pendidikan'),
            'alamat' => $this->request->getVar('alamat'),
            'kec' => $this->request->getVar('kec'),
            'kab' => $this->request->getVar('kab'),
            'kodepos' => $this->request->getVar('kodepos'),
            'asal' => $this->request->getVar('asal'),
            'handphone' => $this->request->getVar('handphone'),
            'email' => $this->request->getVar('email'),
            'thn_gabung' => $this->request->getVar('thn_gabung'),
            'laznah' => $this->request->getVar('laznah'),
            'status_rda' => $this->request->getVar('status_rda'),
            'grade' => $this->request->getVar('grade'),
            'golongan' => $this->request->getVar('golongan'),
            'sub_golongan' => $this->request->getVar('sub_golongan'),
            'jabatan' => $this->request->getVar('jabatan'),
            'status_santri' => $this->request->getVar('status_santri')
        ]);

        $santriBaru =  $this->santriModel->where('nip', $this->request->getVar('nip'))->first();
        $id = ['santri' => $santriBaru['id']];

        $tunjanganBaru = [
            'id' => $id['santri'],
            'nip' => $this->request->getVar('nip'),
            'nama' => $this->request->getVar('nama'),
        ];

        $potonganBaru = [
            'id' => $id['santri'],
            'nip' => $this->request->getVar('nip'),
            'nama' => $this->request->getVar('nama'),
        ];

        $this->tunjanganModel->insert($tunjanganBaru);
        $this->potonganModel->insert($potonganBaru);

        flash('Berhasil', 'Menambah Data Pengurus..');
        return redirect()->to('/pengurus/' . $id['santri']);
    }

    public function update($id)
    {
        $this->santriModel->save([
            'id' => $id,
            'nip' => $this->request->getVar('nip'),
            'no_absen' => $this->request->getVar('no_absen'),
            'ktp' => $this->request->getVar('ktp'),
            'gender' => $this->request->getVar('gender'),
            'nama' => $this->request->getVar('nama'),
            'panggilan' => $this->request->getVar('panggilan'),
            'tmp_lahir' => $this->request->getVar('tmp_lahir'),
            'tgl_lahir' => $this->request->getVar('tgl_lahir'),
            'gol_darah' => $this->request->getVar('gol_darah'),
            'status' => $this->request->getVar('status'),
            'pasangan' => $this->request->getVar('pasangan'),
            'jml_istri' => $this->request->getVar('jml_istri'),
            'jml_anak' => $this->request->getVar('jml_anak'),
            'pendidikan' => $this->request->getVar('pendidikan'),
            'alamat' => $this->request->getVar('alamat'),
            'kec' => $this->request->getVar('kec'),
            'kab' => $this->request->getVar('kab'),
            'kodepos' => $this->request->getVar('kodepos'),
            'asal' => $this->request->getVar('asal'),
            'handphone' => $this->request->getVar('handphone'),
            'email' => $this->request->getVar('email'),
            'thn_gabung' => $this->request->getVar('thn_gabung'),
            'laznah' => $this->request->getVar('laznah'),
            'status_rda' => $this->request->getVar('status_rda'),
            'grade' => $this->request->getVar('grade'),
            'golongan' => $this->request->getVar('golongan'),
            'sub_golongan' => $this->request->getVar('sub_golongan'),
            'jabatan' => $this->request->getVar('jabatan'),
            'status_santri' => $this->request->getVar('status_santri'),
            'acc' => false
        ]);

        $santri = $this->santriModel->find($id);
        $oldName = ['santri' => $santri];

        if ($oldName['santri'] != $this->request->getVar('nama')) {
            $data = ['nama' => $this->request->getVar('nama')];
            $this->tunjanganModel->update($id, $data);
            $this->potonganModel->update($id, $data);
        }

        flash('Berhasil', 'Mengubah Data Pengurus..');
        return redirect()->to('/pengurus/' . $id);
    }

    public function delete($id)
    {
        $santri = $this->santriModel->find($id);
        // hapus gambar
        if (file_exists(FCPATH . 'assets/images/pengurus/' . $santri['nip'] . '.png')) {
            unlink('assets/images/pengurus/' . $santri['nip'] . '.png');
        }

        $this->santriModel->delete($id);
        $this->tunjanganModel->delete($id);
        $this->potonganModel->delete($id);
        flash('Berhasil', 'Menghapus Data Pengurus..');
        return redirect()->to('/pengurus');
    }

    public function upload($nip)
    {
        if (isset($_POST["image"])) {
            $data = $_POST["image"];

            ////data:image/png;base64,

            $image_array_1 = explode(";", $data);

            //base64,

            $image_array_2 = explode(",", $image_array_1[1]);

            

            $data = base64_decode($image_array_2[1]);

            $destdir = base_url() . '/assets/images/pengurus/';

            $imageName = $nip . '.png';

            file_put_contents($destdir . $imageName, $data);

            // flash('Berhasil', 'Menyimpan Foto Profil');

            echo '<img src="/assets/images/pengurus/' . $imageName . '" class="img-fluid rounded-circle mb-2" id="imgPreview" width="130" height="130" />';
        }
    }

    public function export()
    {
        $santri = $this->santriModel->where('status_santri <> ', 'Suspend')->findAll();

        $spreadsheet = new Spreadsheet();
        $sheet = $spreadsheet->getActiveSheet();
        $sheet->setCellValue('A1', 'No');
        $sheet->setCellValue('B1', 'nip');
        $sheet->setCellValue('C1', 'no_absen');
        $sheet->setCellValue('D1', 'ktp');
        $sheet->setCellValue('E1', 'gender');
        $sheet->setCellValue('F1', 'nama');
        $sheet->setCellValue('G1', 'panggilan');
        $sheet->setCellValue('H1', 'tmp_lahir');
        $sheet->setCellValue('I1', 'tgl_lahir');
        $sheet->setCellValue('J1', 'gol_darah');
        $sheet->setCellValue('K1', 'status');
        $sheet->setCellValue('L1', 'pasangan');
        $sheet->setCellValue('M1', 'jml_istri');
        $sheet->setCellValue('N1', 'jml_anak');
        $sheet->setCellValue('O1', 'pendidikan');
        $sheet->setCellValue('P1', 'alamat');
        $sheet->setCellValue('Q1', 'kec');
        $sheet->setCellValue('R1', 'kab');
        $sheet->setCellValue('S1', 'kodepos');
        $sheet->setCellValue('T1', 'asal');
        $sheet->setCellValue('U1', 'handphone');
        $sheet->setCellValue('V1', 'email');
        $sheet->setCellValue('W1', 'thn_gabung');
        $sheet->setCellValue('X1', 'laznah');
        $sheet->setCellValue('Y1', 'status_rda');
        $sheet->setCellValue('Z1', 'grade');
        $sheet->setCellValue('AA1', 'golongan');
        $sheet->setCellValue('AB1', 'sub_golongan');
        $sheet->setCellValue('AC1', 'jabatan');
        $sheet->setCellValue('AD1', 'status_santri');

        $col = 2;
        foreach ($santri as $san) {
            $sheet->setCellValue('A' . $col, ($col - 1));
            $sheet->setCellValue('B' . $col, $san['nip']);
            $sheet->setCellValue('C' . $col, $san['no_absen']);
            $sheet->setCellValue('D' . $col, $san['ktp']);
            $sheet->setCellValue('E' . $col, $san['gender']);
            $sheet->setCellValue('F' . $col, $san['nama']);
            $sheet->setCellValue('G' . $col, $san['panggilan']);
            $sheet->setCellValue('H' . $col, $san['tmp_lahir']);
            $sheet->setCellValue('I' . $col, $san['tgl_lahir']);
            $sheet->setCellValue('J' . $col, $san['gol_darah']);
            $sheet->setCellValue('K' . $col, $san['status']);
            $sheet->setCellValue('L' . $col, $san['pasangan']);
            $sheet->setCellValue('M' . $col, $san['jml_istri']);
            $sheet->setCellValue('N' . $col, $san['jml_anak']);
            $sheet->setCellValue('O' . $col, $san['pendidikan']);
            $sheet->setCellValue('P' . $col, $san['alamat']);
            $sheet->setCellValue('Q' . $col, $san['kec']);
            $sheet->setCellValue('R' . $col, $san['kab']);
            $sheet->setCellValue('S' . $col, $san['kodepos']);
            $sheet->setCellValue('T' . $col, $san['asal']);
            $sheet->setCellValue('U' . $col, $san['handphone']);
            $sheet->setCellValue('V' . $col, $san['email']);
            $sheet->setCellValue('W' . $col, $san['thn_gabung']);
            $sheet->setCellValue('X' . $col, $san['laznah']);
            $sheet->setCellValue('Y' . $col, $san['status_rda']);
            $sheet->setCellValue('Z' . $col, $san['grade']);
            $sheet->setCellValue('AA' . $col, $san['golongan']);
            $sheet->setCellValue('AB' . $col, $san['sub_golongan']);
            $sheet->setCellValue('AC' . $col, $san['jabatan']);
            $sheet->setCellValue('AD' . $col, $san['status_santri']);
            $col++;
        }

        $sheet->getStyle('A1:AD1')->getFont()->setBold(true);
        $sheet->getStyle('A1:AD1')->getFill()
            ->setFillType(Fill::FILL_SOLID)
            ->getStartColor()->setARGB('FFFFFF00');
        $styleArray = [
            'borders' => [
                'allBorders' => [
                    'borderStyle' => Border::BORDER_THIN,
                    'color' => ['argb', 'FF000000'],
                ],
            ],
        ];
        $sheet->getStyle('A1:AD' . ($col - 1))->applyFromArray($styleArray);

        $sheet->getColumnDimension('B')->setAutoSize(true);
        $sheet->getColumnDimension('F')->setAutoSize(true);
        $sheet->getColumnDimension('G')->setAutoSize(true);
        $sheet->getColumnDimension('H')->setAutoSize(true);
        $sheet->getColumnDimension('I')->setAutoSize(true);
        $sheet->getColumnDimension('O')->setAutoSize(true);
        $sheet->getColumnDimension('U')->setAutoSize(true);
        $sheet->getColumnDimension('V')->setAutoSize(true);
        $sheet->getColumnDimension('W')->setAutoSize(true);
        $sheet->getColumnDimension('X')->setAutoSize(true);
        $sheet->getColumnDimension('Y')->setAutoSize(true);
        $sheet->getColumnDimension('Z')->setAutoSize(true);
        $sheet->getColumnDimension('AA')->setAutoSize(true);
        $sheet->getColumnDimension('AB')->setAutoSize(true);
        $sheet->getColumnDimension('AC')->setAutoSize(true);
        $sheet->getColumnDimension('AD')->setAutoSize(true);

        $writer = new Xlsx($spreadsheet);
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment;filename=pengurus.xlsx');
        header('Cache-Control: max-age=0');
        $writer->save('php://output');
        exit();
    }

    public function import()
    {
        $file = $this->request->getFile('file');
        $ext = $file->getClientExtension();
        if ($ext == 'xls' || $ext == 'xlsx') {
            if ($ext == 'xls') {
                $reader = new \PhpOffice\PhpSpreadsheet\Reader\Xls();
            } else {
                $reader = new \PhpOffice\PhpSpreadsheet\Reader\Xlsx();
            }
            $spreadsheet = $reader->load($file);
            $santri = $spreadsheet->getActiveSheet()->toArray();
            foreach ($santri as $key => $san) {
                if ($key == 0) {
                    continue;
                }
                $data = [
                    'nip' => $san[1],
                    'no_absen' => $san[2],
                    'ktp' => $san[3],
                    'gender' => $san[4],
                    'nama' => $san[5],
                    'panggilan' => $san[6],
                    'tmp_lahir' => $san[7],
                    'tgl_lahir' => $san[8],
                    'gol_darah' => $san[9],
                    'status' => $san[10],
                    'pasangan' => $san[11],
                    'jml_istri' => $san[12],
                    'jml_anak' => $san[13],
                    'pendidikan' => $san[14],
                    'alamat' => $san[15],
                    'kec' => $san[16],
                    'kab' => $san[17],
                    'kodepos' => $san[18],
                    'asal' => $san[19],
                    'handphone' => $san[20],
                    'email' => $san[21],
                    'thn_gabung' => $san[22],
                    'laznah' => $san[23],
                    'status_rda' => $san[24],
                    'grade' => $san[25],
                    'golongan' => $san[26],
                    'sub_golongan' => $san[27],
                    'jabatan' => $san[28],
                    'status_santri' => $san[29],
                ];
                $this->santriModel->insert($data);
            }
            // dd($data);
            flash('Berhasil', 'Import Data santri..', 'success');
            return redirect()->to('/pengurus');
        } else {
            flash('Gagal', 'File tidak diketahui..', 'warning');
            return redirect()->to('/pengurus');
        }
    }
}
